    ...
    def check_auto_close_positions(self):
        ...
        for symbol, tracker in self.position_trackers.items():
            ...
            if self.margin_calls > 0:
                margin_call_threshold = 0.8 * self.default_stop_loss
                if pnl_percent <= margin_call_threshold:
                    current_qty = position.get('currentQty', 0)
                    if current_qty < 0:
                        side = 'sell'
                    else:
                    ...
                    if position_details:
                        print(f"Opened additional position for {symbol} due to margin call.")
                        self.sound_open_position.play_sound()
                        self.margin_calls -= 1
                        self.margin_calls_input.setText(str(self.margin_calls))
                        tracker['used_margin_calls'] = tracker.get('used_margin_calls', 0) + 1
                        self.update_used_margin_calls_label(tracker['used_margin_calls'])
                        self.save_configurations()
                    else:
                        print("Failed to open additional position due to margin call.")
